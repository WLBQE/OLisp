...
let check toplevels =
  let confirm_type typ (ret, _) =
    if ret = VarType typ then typ else
      raise (Failure (string_of_ret_typ ret ^ " and " ^ string_of_typ typ ^ " do not match"))
  in
  let confirm_ret_type ret_typ (ret, _) =
    if ret = ret_typ then ret_typ else
      raise (Failure (string_of_ret_typ ret ^ " and " ^ string_of_ret_typ ret_typ ^ " do not match"))
  in
  let rec check_call lamb args = ((match lamb with
      (VarType (Lambda (types, ret)), _) -> (try ignore (List.map2 confirm_type types args)
    ...
    | (BuiltIn builtin, _) -> let check_same_type exprs = match exprs with
      ...
      (match builtin with
        Add | Mult -> (match check_same_type args with
          VarType Int :: _ when List.length args > 1 -> VarType Int
        | VarType Double :: _ when List.length args > 1 -> VarType Double
      ...
      | And | Or -> (match check_same_type args with
          VarType Bool :: _ when List.length args > 1 -> VarType Bool
        | _ -> raise (Failure (string_of_built_in builtin ^ ": invalid arguments")))
      | Not -> (match args with
  ...
  | LambdaExpr (typs, ret, formals, expr) -> (VarType (Lambda (typs, ret)),
    ...
    in
    let expr' = check_expr (List.rev (sym' :: List.rev syms)) cls expr in
